...
class SPLUtils
{
     ...
     */
    static function simplexml_dump(SimpleXMLElement $sxml, $return=false)
    {
        $indent = "\t";
     ...
     * Equally, no warranty: don't blame me if your aircraft or nuclear power plant fails because of this code!
     */
    static function simplexml_tree(SimpleXMLElement $sxml, $include_string_content=false, $return=false)
    {
        $indent = "\t";
        $content_extract_size = 25;
        // Get all the namespaces declared at the *root* of this document
        // All the items we're looking at are in the same document, so we only need do this once
        $doc_ns = $sxml->getDocNamespaces(false);
        ...
        while ( isset($sxml[$root_item_index]) )
        {
            ...
            else
            {
                ...
                $dump .= self::_simplexml_tree_recursively_process_node(
                    $root_item, 1,
     ...
     */
    static function _simplexml_tree_recursively_process_node($item, $depth, $include_string_content, $indent, $content_extract_size)
    {
        $dump = '';
